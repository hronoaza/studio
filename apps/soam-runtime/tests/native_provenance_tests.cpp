...
static_assert(!std::is_default_constructible_v<ProductionRelationshipProvenance>);
...
int main() {
    ...
    require(!mesh.captureProductionRelationshipProvenance({0, 1}).has_value());
    require(!mesh.captureProductionRelationshipProvenance({0, 0}).has_value());
    require(!mesh.captureProductionRelationshipProvenance({0, 99}).has_value());

    mesh.connectNodes(0, 1);

    const auto first = mesh.captureProductionRelationshipProvenance({0, 1});
    ...
    const auto same = mesh.captureProductionRelationshipProvenance({0, 1});
    require(same.has_value());
    require(same->relationshipGeneration() == first->relationshipGeneration());
    require(same->stateVersion() == first->stateVersion());

    const auto reverse = mesh.captureProductionRelationshipProvenance({1, 0});
    ...
    const auto afterShock = mesh.captureProductionRelationshipProvenance({0, 1});
    ...
    const auto afterStep = mesh.captureProductionRelationshipProvenance({0, 1});
