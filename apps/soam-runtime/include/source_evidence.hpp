...
enum class SourceEvidenceRetentionResult {
    Inserted,
    AlreadyPresent,
    IntegrityConflict
};

enum class SourceResolutionStatus {
    Found,
    NotFound,
    ResolverFailure,
    IntegrityConflict
...
public:
    ...
    [[nodiscard]] const SourceCaptureId& sourceCaptureId() const noexcept { return sourceCaptureId_; }
    ...
    [[nodiscard]] std::uint64_t relationshipGeneration() const noexcept { return relationshipGeneration_; }
...
private:
    explicit RetainedSourceEvidenceRecord(
    ...
    friend class SourceEvidenceStore;
};
...
class SourceEvidenceStore final : public SourceEvidenceResolver {
public:
