...
public:
    [[nodiscard]]
    const ProductionTransitionRequestBinding& context() const noexcept { return context_; }
...
private:
    constexpr PermissionPrerequisiteEvidence(
        ...
        : context_(context), satisfied_(satisfied) {}
...
public:
    [[nodiscard]]
    ProductionTransitionEligibility eligibility() const noexcept { return eligibility_; }

    [[nodiscard]]
    EligibilityRejectionReason rejectionReason() const noexcept { return rejectionReason_; }

    [[nodiscard]]
    const ProductionTransitionRequestBinding& binding() const noexcept { return binding_; }
